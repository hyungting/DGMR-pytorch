...
class SpatialAttention(nn.Module):
    ...
    """
    def __init__(self, in_channels=192, out_channels=192, kernel_size=3):
        super(SpatialAttention, self).__init__()

        self.conv_in = nn.utils.spectral_norm(nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=1, padding=kernel_size//2))
        self.conv_out = nn.utils.spectral_norm(nn.Conv2d(2, out_channels, kernel_size, stride=1, padding=kernel_size//2))
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        x = self.conv_in(x) # channel=192
        avg_x = torch.mean(x, dim=1, keepdim=True)
        max_x, _ = torch.max(x, dim=1, keepdim=True)
        attn = torch.cat((avg_x, max_x), dim=1) # channel=2
...
class LBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1x1 = nn.utils.spectral_norm(nn.Conv2d(in_channels, out_channels-in_channels, 1, 1, 0))
